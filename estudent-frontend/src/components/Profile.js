...
function Profile(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:1337/getStudent/${id}`);
  ...
  const handlePasswordFormSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`http://localhost:1337/changePassword/${id}`, passwordFormData);
