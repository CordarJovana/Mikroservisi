...
function ExamRegistration(props) {
    ...
    useEffect(() => {
        axios.get(`http://localhost:1337/getActiveExamRegistrations/${id}`)
            .then(response => {
                    ...
                    return {
                        ...
                        'Actions': (
                            <button className="exam-registration-button" onClick={() => {
                                ...
                                const studentId = id;
                                axios.post(`http://localhost:9000/deletePrijava`, { ispitId, studentId })
                                    .then(response => {
                    ...
                    return {
                        ...
                        'Actions': (
                            <button className="exam-registration-button" onClick={() => {
                                const ispitId = registration.id;
                                const studentId = id;
                                axios.post('http://localhost:9000/createNewPrijava', { ispitId, studentId })
                                    .then(response => {
                                        console.log(response.data);
                                        setRefresh(!refresh);
                                    }).catch(error => console.error(error));
